"""
构建 latest_ranks.json（女频）/ latest_ranks_male.json（男频）：
1. 按 --gender 加载对应频道的最近两天 JSON 快照
2. 按分类对比趋势（新上榜/掉榜/排名变化/阅读量变化）
   仅有一天数据时改为「当前在读总量」口径的首日快照摘要
3. 可选调用 OpenAI 兼容 API（如 DeepSeek）生成 AI 总结
4. 输出 latest_ranks[_male].json + trends[_male]/YYYY-MM-DD.json
   + market_summary[_male].json + api/lastest[_male]/*
"""
import os
import re
import json
...
# Windows 中文环境（GBK 控制台）下打印 emoji/中文会抛 UnicodeEncodeError，
# 统一把 stdout/stderr 强制为 UTF-8。
try:
...
def count_theme_hits(books: list, limit: int = 5) -> list:
    """统计一批书（书名+简介）里的题材关键词频次。"""
    counter = {}
    for book in books:
...
def generate_first_day_summary(cat_name: str, cat: dict) -> str:
    """首日快照没有对比基准，改为基于当前榜单本身给出可读结论。

    旧实现在这种情况下只写「首日数据，暂无趋势对比。」，
    男频第一次抓到数据时整站都是这句话，等于没有分析。
    """
    books = cat.get("books", [])
    if not books:
        return "首日数据，暂无趋势对比。"
    ...
    top_titles = "、".join(
        f"《{b.get('title', '未知')}》" for b in books[:3]
    )
    parts = [f"首日快照，暂无昨日对比；头部作品：{top_titles}"]

    themes = count_theme_hits(books)
    if themes:
        parts.append(f"高频题材：{'、'.join(themes)}")

    top_reads = books[0].get("reads", "")
    if top_reads and top_reads != "未知":
        parts.append(f"榜首在读 {top_reads}")
    parts.append(f"入榜 {len(books)} 本")
    return "；".join(parts) + "。"
...
# ---------- 频道配置：女频 / 男频各自独立的赛道分组与题材词库 ----------
# 之前只有女频赛道，导致男频 19 个分类里只有 3 个能归组，全站热点几乎空白。
GENDER_LABELS = {"female": "女频", "male": "男频"}

GENRE_GROUPS_BY_GENDER = {
    "female": [
...
# 当前生效的配置（由 set_gender() 切换，保持既有函数签名不变）
GENRE_GROUPS = GENRE_GROUPS_BY_GENDER["female"]
MARKET_KEYWORDS = MARKET_KEYWORDS_BY_GENDER["female"]


def set_gender(gender: str) -> str:
    """切换当前生效的综合赛道分组与题材关键词词库。"""
    global GENRE_GROUPS, MARKET_KEYWORDS
    gender = "male" if gender == "male" else "female"
    GENRE_GROUPS = GENRE_GROUPS_BY_GENDER[gender]
    MARKET_KEYWORDS = MARKET_KEYWORDS_BY_GENDER[gender]
    return gender


def gender_paths(base_dir: str, gender: str) -> dict:
    """按频道返回输出路径。

    女频沿用历史路径（保持既有前端/接口/外部调用兼容）；
    男频使用 *_male 独立路径，避免两个频道互相覆盖。
    """
    ...
    if gender == "male":
        return {
            "data_dir": data_dir,
            "trends_dir": os.path.join(data_dir, "trends_male"),
            "latest": os.path.join(data_dir, "latest_ranks_male.json"),
            "market": os.path.join(data_dir, "market_summary_male.json"),
            "dates": os.path.join(data_dir, "dates_male.json"),
            "api_dir": os.path.join(api_root, "lastest_male"),
            "api_index": os.path.join(api_root, "lastest_male.json"),
            "api_prefix": "api/lastest_male",
        }
    return {
        "data_dir": data_dir,
        "trends_dir": os.path.join(data_dir, "trends"),
        "latest": os.path.join(data_dir, "latest_ranks.json"),
        "market": os.path.join(data_dir, "market_summary.json"),
        "dates": os.path.join(data_dir, "dates.json"),
        "api_dir": os.path.join(api_root, "lastest"),
        "api_index": os.path.join(api_root, "lastest.json"),
        "api_prefix": "api/lastest",
    }
...
def _save_trends_incremental(trend_path: str, date: str,
    ...
    atomic_write_json(trend_path, trend_output)
...
def atomic_write_json(path: str, payload: dict):
    ...
    直接 open(path, 'w') + json.dump 存在缺陷：若进程在 dump 中途被杀或
    抛异常，目标文件会被清空/截断成 0 字节（本仓库曾因此把
    latest_ranks*.json 写成空文件）。先写 tmp 文件再原子替换，
    要么保留旧文件、要么得到完整新文件，绝不留下 0 字节主文件。
    """
    ...
    fd, tmp_path = _tempfile.mkstemp(
        prefix=".fq-", suffix=".tmp", dir=dir_name, text=True
    )
    try:
...
def write_json(path: str, payload: dict):
    """统一写 JSON，确保中文可读（原子写入，防止截断/0 字节）。"""
    atomic_write_json(path, payload)


def build_lastest_api(output: dict, base_dir: str, paths: dict = None):
    """生成静态 lastest 数据接口。

    GitHub Pages 不支持动态 query API，因此这里将 type 参数映射为静态文件：
    - api/lastest/all.json：全量数据（女频）
    - api/lastest/<type>.json：单个类型数据
    - api/lastest.json / api/lastest/index.json：类型索引
    男频写入 api/lastest_male/*，与女频完全隔离。
    """
    api_root = os.path.join(base_dir, "api")  # noqa: F841 (保留用于兼容旧调用)
    if paths is None:
        paths = gender_paths(base_dir, "female")
    lastest_dir = paths["api_dir"]
    api_prefix = paths.get("api_prefix", "api/lastest")
    ...
    gender = output.get("gender", "female")
    ...
    for cat in categories:
        ...
        url = f"{api_prefix}/{quote(filename)}.json"
        types.append({
            "type": type_name,
            "url": url,
    ...
    write_json(paths["api_index"], index_payload)
...
def collect_snapshot_hot_types(output: dict) -> list:
    """无对比数据（首日）时的兜底：按当前榜单在读总量给分类排序。

    返回结构与其他 hot_types 保持一致，额外带 basis 标记，
    前端据此把「新增在读」显示成「当前在读」。
    """
    result = []
    for cat in output.get("categories", []):
        ...
        if not books:
            continue
        reads_total = sum(parse_reads(b.get("reads", "")) for b in books)
        if reads_total <= 0:
            continue
        result.append({
            "name": cat.get("name", ""),
            "score": round(reads_total),
            "new_count": 0,
            "dropped_count": 0,
            "read_count": len(books),
            "read_growth_total": reads_total,
            "active_days": 1,
            "top_book": books[0].get("title", ""),
            "top_reads": books[0].get("reads", ""),
            "basis": "snapshot",
        })
    return sorted(
...
def collect_snapshot_hot_themes(output: dict) -> list:
    """无对比数据（首日）时的兜底：直接统计当前全部入榜书的题材词。"""
    counter = {}
    for cat in output.get("categories", []):
        ...
        for book in cat.get("books", []):
            ...
            for keyword in MARKET_KEYWORDS:
                if keyword not in text:
                    continue
                item = counter.setdefault(keyword, {"name": keyword,
                                                    "count": 0, "categories": set()})
                item["count"] += 1
                item["categories"].add(cat_name)
    themes = [item for item in counter.values() if item["count"] > 0]
    themes.sort(key=lambda x: (x["count"], len(x["categories"])), reverse=True)
    return [
...
def build_rule_market_summary(period_label: str, hot_genres: list,
                              hot_types: list, hot_themes: list,
                              basis: str = "growth") -> str:
    """基于统计结果生成全站热点兜底文案。"""
    ...
    if basis == "snapshot":
        return (
            f"{period_label}暂无对比数据，按当前在读总量看，"
            f"{top_genres or top_types} 更集中，具体分类以 {top_types} 领先；"
            f"题材上 {top_themes} 高频。"
            f"（首日快照口径，积累两天后即可给出增长类结论）"
        )
    return (
...
def build_market_summary_payload(output: dict, trends_dir: str) -> dict:
    ...
    for key, days in MARKET_PERIODS:
        ...
        basis = "growth"
        if not hot_types:
            # 首日或对比数据缺失时退化为「当前在读总量」口径，
            # 否则男频第一次抓取时整块热点板都是空的。
            hot_types = collect_snapshot_hot_types(output)
            basis = "snapshot"
        ...
        if not hot_themes and basis == "snapshot":
            hot_themes = collect_snapshot_hot_themes(output)
        fallback_summary = build_rule_market_summary(
            period_label, hot_genres, hot_types, hot_themes, basis
        )
        periods[key] = {
            "period": period_label,
            "source": "rule",
...
def build_market_ai_prompt(payload: dict) -> str:
    """构建全站热点 AI 总结 prompt。"""
    gender_label = GENDER_LABELS.get(payload.get("gender", "female"), "女频")
    sections = []
    for key, data in payload.get("periods", {}).items():
        ...
        basis_tag = "（首日快照口径：当前在读总量）" if data.get("basis") == "snapshot" else ""
        sections.append(
            f"周期 {key} / {data['period']}{basis_tag}:\n"
    ...
    return f"""你是一位网文市场编辑，请根据番茄{gender_label}新书榜的统计结果，为每个周期生成一段全站热点判断。
...
3. 综合赛道和具体分类必须按“新增在读/阅读增长”解读，不要写“榜单动能”或“排名动能”；若该周期标注了「首日快照口径」，则改按“当前在读总量”解读，并说明这是首日快照、再积累一天才有增长结论。
...
def enrich_market_summary_with_ai(payload: dict, api_key: str,
    ...
    注意：推理型模型（如 deepseek-v4-flash）会先消耗「思考 token」再输出正文，
    max_tokens 给小了就会出现「有 usage 但 content 为空」，这里逐次加大预算重试。
    """
    try:
    ...
    token_budgets = [6000, 12000, 20000]
    ...
    for attempt, budget in enumerate(token_budgets, start=1):
        try:
            ...
            if not content or not content.strip():
                raise ValueError(f"API 返回空内容（max_tokens={budget}，可能被思考 token 占满）")
...
def load_env_file(base_dir: str):
    """从项目根目录 .env 读取 AI 配置（不覆盖已存在的环境变量）。

    本地一键更新脚本不再需要手动 set 环境变量，也不会把密钥写进仓库。
    """
    ...
    try:
        ...
        print(f"✅ 已加载 AI 配置: {env_path}")
    except Exception as e:
        print(f"⚠️  读取 .env 失败（忽略）: {e}")
...
def main():
    ...
    parser.add_argument("--gender", choices=["female", "male"], default="female",
                        help="数据频道：female=女频（默认），male=男频")
    parser.add_argument("--no-ai", action="store_true",
                        help="跳过 AI，只用规则统计（省钱/离线可用；已有 AI 总结会保留）")
    args = parser.parse_args()

    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    load_env_file(base_dir)
    gender = set_gender(args.gender)
    gender_label = GENDER_LABELS[gender]
    paths = gender_paths(base_dir, gender)

    data_dir = os.path.join(base_dir, "data")
    trends_dir = paths["trends_dir"]
    os.makedirs(trends_dir, exist_ok=True)

    print(f"📡 频道: {gender_label}（{gender}）")
    print(f"   输出: {os.path.basename(paths['latest'])} / "
          f"{os.path.basename(paths['market'])} / "
          f"{os.path.basename(trends_dir)}/")

    # 查找 JSON 快照文件（按频道前缀）
    snapshot_prefix = "fanqie_male_new_ranks_*.json" if args.gender == "male" \
        else "fanqie_female_new_ranks_*.json"
    snapshots = sorted(
        glob.glob(os.path.join(data_dir, snapshot_prefix))
    )
    ...
    if args.date:
        ...
        target_name = "fanqie_male_new_ranks_" if args.gender == "male" \
            else "fanqie_female_new_ranks_"
        target_path = os.path.join(
            data_dir, f"{target_name}{target_date_compact}.json"
        )
    ...
    if prev_data:
        ...
        print("仅有一天数据，无法生成趋势对比；改用首日榜单摘要。")
        trends = {
            cat["name"]: {
                "new_count": 0,
                "dropped_count": 0,
                ...
                "top_fallers": [],
                "reads_growth": [],
                "summary": generate_first_day_summary(cat["name"], cat),
            }
    ...
    output = {
        ...
        "prev_date": prev_date,
        "gender": gender,           # female | male，供前端选择快照文件名前缀
    ...
    # 写入 latest_ranks.json（女频）/ latest_ranks_male.json（男频）
    out_path = paths["latest"]
    atomic_write_json(out_path, output)
    print(f"\n✅ 已生成: {out_path}")

    # 生成静态 API 文件
    api_dir = build_lastest_api(output, base_dir, paths)
    ...
    elif args.no_ai and os.path.exists(market_path):
        # --no-ai 只是跳过本次 AI 调用，不应把同一天已有的 AI 热点总结降级成规则文案
        try:
            old_market = load_snapshot(market_path)
            if old_market.get("date") == market_payload.get("date"):
                kept = 0
                for key, item in (old_market.get("periods") or {}).items():
                    if item.get("source") != "ai":
                        continue
                    if key in market_payload["periods"] and item.get("summary"):
                        market_payload["periods"][key]["summary"] = item["summary"]
                        market_payload["periods"][key]["source"] = "ai"
                        kept += 1
                if kept:
                    print(f"  ↩️  --no-ai：保留同日已有的 {kept} 段 AI 热点总结")
        except Exception as e:
            print(f"  ⚠️  读取旧热点总结失败（忽略）: {e}")
    ...
    atomic_write_json(dates_path, {"gender": gender, "dates": sorted(date_list)})
