document.addEventListener('DOMContentLoaded', () => {
    ...
    async function init() {
        ...
        try {
            // 频道（女频/男频）由 URL ?gender= 或 localStorage 决定，路径见 js/channel.js
            const CH = window.FQ.paths();
            window.FQ.applyTitle();

            const dateIndex = await fetchJson(`${CH.dates}?${cacheBuster}`);
            const dates = (dateIndex.dates || []).slice().sort().slice(-maxDays);
            const snapshots = await Promise.all(
                dates.map(date => fetchJson(`${CH.snapshot(date)}?${cacheBuster}`).catch(() => null))
            );
