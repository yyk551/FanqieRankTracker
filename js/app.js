document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentGender = CH.gender;      // 'female' | 'male'，来自频道配置
    ...
    if (sidebarSubtitle) sidebarSubtitle.textContent = `${CH.label}新书榜追踪`;
    ...
    function loadLatestData() {
        // 主数据源 latest_ranks.json（内容最新）；若缺失/为空/解析失败，
        // 自动回退到静态接口 api/lastest/all.json（结构同构，已随构建同步生成）
        return fetchJsonSafe(`${CH.latest}?${cacheBuster}`)
            .then(data => {
                if (!data || !Array.isArray(data.categories) || data.categories.length === 0) {
                    throw new Error('latest 数据为空或结构异常');
                }
                return data;
            })
            .catch(err => {
                console.warn(`加载 ${CH.latest} 失败（${err.message}），回退到 ${CH.apiAll}`);
                return fetchJsonSafe(`${CH.apiAll}?${cacheBuster}`).then(data => {
                    if (!data || !Array.isArray(data.categories) || data.categories.length === 0) {
                        throw new Error('备选数据源也不可用');
                    }
                    return data;
                });
            })
            .then(data => {
                allData = data;
                currentGender = data.gender === 'male' ? 'male'
                    : (data.gender === 'female' ? 'female' : CH.gender);
    ...
    function loadDateData(dateStr) {
        // dateStr = "YYYY-MM-DD"，路径由当前频道决定（见 js/channel.js）
        const isLatest = currentDateIndex === availableDates.length - 1;
        ...
        const snapshotUrl = `${CH.snapshot(dateStr)}?${cacheBuster}`;
        const trendUrl = `${CH.trendFile(dateStr)}?${cacheBuster}`;
    ...
    function renderBooks(cat) {
        ...
        books.forEach((book, index) => {
            ...
            card.href = bookId ? `book.html?id=${encodeURIComponent(bookId)}&gender=${CH.gender}` : 'javascript:void(0)';
            card.rel = 'noopener';
